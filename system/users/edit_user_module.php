...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {  //data pass through URL 
    ...
    extract($_GET);
    $db = dbConn();
    $sql = "SELECT * FROM user_modules WHERE UserId='$userid'";

    $result = $db->query($sql);
    $row = $result->fetch_assoc();

    $UserId = $row['UserId'];   //assign variable from recent data
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($Name)) {
        $db = dbConn();
        $sql = "SELECT * FROM user_modules WHERE UserId='$UserId' AND ModuleId='$ModuleId'";
        $result = $db->query($sql);
...
<div class="row">
    <div class="col-12">
        <a href="<?= SYS_URL ?>users/user_role.php" class="btn bg-warning mb-2 btn-sm"><i class="fas fa-plus-circle"></i> View
            User role</a>
                    ...
                    <div class="col-lg-6">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="ModuleId" name="ModuleId">
                                <option value="">--</option>
                                <?php while ($row = $result->fetch_assoc()) { ?>
                                <option value="<?= $row['ModuleId'] ?>" <?= @$ModuleId==$row['ModuleId']?'selected':'' ?>>
                                    <?= $row['Name'] ?></option>
                                <?php } ?>
                            </select>
